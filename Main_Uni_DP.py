...
from data_feed_Sequence_Uni_DP import DataFeed  as DF_DP
import numpy as np
import os
import scipy.io
from Network_Model import Uni_DP_net
...
train_loader = DataLoader(DF_DP(train_dir, nat_sort=True, transform=proc_pipe),
                        batch_size=BATCH_SIZE,
                        shuffle=False,
                        drop_last=True)

net = Uni_DP_net(256, 1, 1)
net.cuda()
...
for epoch in range(EPOCH):
    for tr_count, (d1, d2, label) in enumerate(train_loader):
        count_epoch += 1
        # Depth maps
        ...
        label[0] = label[0] * 10
        label[0] = (label[0] - mean_label_X_Train) / std_label_X_Train  # 均值方差归一化
...
test_loader = DataLoader(DF_DP(test_dir, nat_sort=True, transform=proc_pipe),
...
num = 0
x_l = []
for tr_count, (d1, d2, label) in enumerate(test_loader):
    count_epoch += 1
    d1 = d1.type(torch.cuda.FloatTensor)
    d2 = d2.type(torch.cuda.FloatTensor)
    z1 = d1.cuda()
    z2 = d2.cuda()
    Final_out = net.forward(z1, z2)
...
# Save the prediction results in your folder.
filenameDP = 'MMFF_Uni_DP_result.mat'
scipy.io.savemat("Result/"+filenameDP, mdict={'result_x': resultxreal,'result_y': resultyreal,'label_x': labelxreal, 'label_y': labelyreal})
